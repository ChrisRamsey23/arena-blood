'use strict';
// ═══════════════════════════════════════════════════════════════
// ARENA BLOOD V3 — script.js
// All UI functions on central UI object (fixes V2 scope bugs)
// ═══════════════════════════════════════════════════════════════
...
const ITEMS = {
  ...
  str_draught:     {id:'str_draught',     name:'Strength Draught',   slot:null,    tier:0, icon:'💪', buffAtk:8, buffTurns:4, stats:{}, desc:'+8 ATK 4 turns', price:58, type:'consumable'},
};

// Shop stock by player level — show relevant tier only
...
const ENEMY_POOL = [
  ...
  // Tier 2
  {id:'scarred_brigand',  name:'Scarred Brigand',     sprite:'warrior',  tier:2, bHp:90,  atk:18, def:7,  spd:3, prefRange:0,  hasRanged:false, xp:45, gold:35, diff:'easy'},
  {id:'net_fighter',      name:'Net Fighter',         sprite:'archer',   tier:2, bHp:80,  atk:14, def:6,  spd:4, prefRange:30, hasRanged:true,  xp:48, gold:38, diff:'easy'},
  {id:'arena_rookie',     name:'Arena Rookie',        sprite:'warrior',  tier:2, bHp:100, atk:20, def:8,  spd:3, prefRange:0,  hasRanged:false, xp:55, gold:40, diff:'avg'},
  {id:'mercenary',        name:'Mercenary',           sprite:'warrior',  tier:2, bHp:95,  atk:22, def:6,  spd:4, prefRange:0,  hasRanged:false, xp:58, gold:42, diff:'avg'},
  {id:'scout',            name:'Scout',               sprite:'archer',   tier:2, bHp:75,  atk:18, def:5,  spd:7, prefRange:20, hasRanged:true,  xp:52, gold:36, diff:'avg'},
  {id:'berserker',        name:'Berserker',           sprite:'berserker',tier:2, bHp:110, atk:26, def:5,  spd:4, prefRange:0,  hasRanged:false, xp:65, gold:48, diff:'hard'},
  // Tier 3
  {id:'vet_legionary',    name:'Veteran Legionary',   sprite:'veteran',  tier:3, bHp:140, atk:26, def:15, spd:3, prefRange:0,  hasRanged:false, xp:85, gold:65, diff:'easy'},
  {id:'carthage_archer',  name:'Carthage Archer',     sprite:'archer',   tier:3, bHp:110, atk:20, def:10, spd:5, prefRange:40, hasRanged:true,  xp:88, gold:68, diff:'easy'},
  {id:'barbarian_chief',  name:'Barbarian Chief',     sprite:'berserker',tier:3, bHp:160, atk:30, def:12, spd:3, prefRange:0,  hasRanged:false, xp:95, gold:75, diff:'avg'},
  {id:'mounted_hunter',   name:'Mounted Hunter',      sprite:'archer',   tier:3, bHp:130, atk:28, def:10, spd:6, prefRange:30, hasRanged:true,  xp:92, gold:72, diff:'avg'},
  {id:'death_guard',      name:'Death Guard',         sprite:'veteran',  tier:3, bHp:180, atk:34, def:16, spd:3, prefRange:0,  hasRanged:false, xp:110,gold:90, diff:'hard'},
  // Tier 4
...
function generatePrizeFight(player) {
  ...
  // Stats ≈ 80-90% of player, HP tuned for 10-12 turns
  const ratio   = 0.80 + Math.random() * 0.10;
  const avgHit  = Math.max(1, player.atk * 0.55);
  const hp      = Math.floor(avgHit * 11); // ~11 turns to kill
  ...
  return {
    ...
    name: `${name} ${epithet}`,
    sprite: 'champion',
    hp, maxHp: hp, atk, def, spd: 3 + Math.floor(Math.random() * 3),
...
class Player {
  constructor(name, gender, skin, style) {
    ...
    // Base stats: tuned so net damage = 8-18% of enemy HP per hit
    const bases = {
  ...
  _equipStarter() {
    ...
    if (this.style === 'retiarius') {
      this.addToInventory(ITEMS['throwing_knife'], 1);
    }
  }
...
// ── COMBAT ENGINE ─────────────────────────────────────────────────
// NOTE: CombatEngine calls UI.xxx() — UI must be defined before use
class CombatEngine {
  constructor(player) {
    this.player   = player;
    this.enemy    = null;
  ...
  start(enemy) {
    ...
    // refreshCombat() called by setupCombat() after start() — no call needed here
    if (!this.playerTurn) this._delayEnemy();
  }

  // ── DAMAGE FORMULA ─────────────────────────────────────────
  // Ensures meaningful damage every hit, fights resolve 10-15 turns
  ...
  playerAction(action) {
    ...
    switch (action) {
      ...
      case 'advance':
        ...
        // If we just reached 0, also do a light attack
        if (this.distance === 0) {
  ...
  _pMelee() {
    ...
    if (type === 'miss') {
      ...
      UI.floatDamage(dmg, false, false, true);
      UI.shakeEl('cb-esprite');
    } else {
      this.enemy.hp = Math.max(0, this.enemy.hp - dmg);
  ...
  _pRanged() {
    ...
    const wep = this.player.equipped.ranged?.name || 'ranged';
    ...
    if (type === 'miss') {
      ...
      this.enemy.hp = Math.max(0, this.enemy.hp - dmg);
      UI.log(`💥 RANGED CRIT! ${dmg} damage with ${wep}!`, 'crit');
      UI.floatDamage(dmg, false, false, true);
      UI.shakeEl('cb-esprite');
    } else {
      this.enemy.hp = Math.max(0, this.enemy.hp - dmg);
  ...
  useItem(itemId) {
    ...
    if (item.healAmt) {
      const healed = this.player.heal(Math.min(item.healAmt, this.player.maxHp));
  ...
  _doMercy() {
    ...
    // UI.showMercyOverlay is defined on UI object — no scope issue
  ...
  // ── ENEMY AI ───────────────────────────────────────────────
  // Rules:
  //  1. Never back up twice in a row
  //  2. After turn 16 → must attack (anti-stall)
  //  3. Use preferred range to guide movement
  _enemyTurn() {
    ...
    // Hard anti-stall: force attack after turn 16
    if (this.turn >= 16) {
      action = d === 0 ? 'attack' : (this.enemy.hasRanged && d > 0 ? 'ranged' : 'advance');
    }
    // Low HP: desperate close-in attack
    ...
    // Prefer range
      ...
      if (pref === 0 || d === 0) {
        action = 'attack';
      } else if (this.enemy.hasRanged && d > 0) {
        action = 'ranged';
      } else {
        action = 'advance';
      }
    ...
    if (action === 'backup') {
      ...
      UI.refreshCombat();
      // If just closed to 0, also light strike
      if (this.distance === 0) {
        this._enemyStrike(false, true); // light
        if (!this.active) return;
      }
      ...
      if (this.distance !== 0) {
        ...
        this._enemyStrike(false, false);
        if (!this.active) return;
      }
    } else if (action === 'ranged') {
      if (this.distance === 0) {
        this._enemyStrike(false, false); // forced melee
      } else {
        this._enemyStrike(true, false);
      }
      if (!this.active) return;
    }
  ...
  _enemyStrike(isRanged, isLight) {
    ...
    if (type === 'miss') {
      ...
      UI.floatDamage(dmg, true, false, true);
      UI.shakeEl('cv-combat');
    } else {
      this.player.hp = Math.max(0, this.player.hp - dmg);
      this.player.damageTaken += dmg;
      UI.log(`${this.enemy.name} ${method} you for ${dmg}.`, 'enemy');
      UI.floatDamage(dmg, true);
      UI.shakeEl('cv-combat');
    }
    UI.refreshCombat();
...
class Shop {
  ...
  restock(level) {
    this.stock = getShopStock(level).map(id => ({...ITEMS[id]}));
  }
  buy(player, itemId) {
...
// ── CANVAS AVATAR ─────────────────────────────────────────────────
// Pixel art gladiator rendered via canvas 2D
const AvatarRenderer = {
  draw(canvasId, player, px = 8) {
    ...
    const weaponIcon = weaponItem?.icon || '👊';

    // Grid: 11 cols × 18 rows, drawn at px scale
    const G = [
      // row 0: helm top
    ...
    const colorMap = {
      'H': helmc || sk.dark,    // helm
      'h': helmc ? this._darken(helmc) : sk.dark,
      'F': sk.base,              // face/skin
      'e': '#1A1008',            // eyes
      'S': shieldc || '#4A6888', // shield arm
      'A': armorc,               // armor
      'a': this._darken(armorc), // armor shadow
      'b': '#2A1808',            // belt
      'W': weaponIcon === '👊' ? sk.mid : '#A0A0A0', // weapon arm color
      'L': sk.mid,               // legs skin
      'G': '#909090',            // greaves (always steel-ish over boots)
      'B': bootsc,               // boots
    };
    ...
    // Weapon icon floating to the right
    if (weaponItem || weaponIcon) {
      ctx.font = `${px * 2.2}px serif`;
      ctx.textAlign = 'left';
      const wx = 11 * px + 2;
      const wy = 9 * px + px * 2;
      if (wx < cv.width) {
        ctx.fillText(weaponItem?.icon || '👊', wx, wy);
      }
    }
  },
  ...
  _helmColor(item) {
    if (!item) return null;
    const m = {iron_helm:'#909090', bronze_galea:'#C87828', imperial_helm:'#C0A010', centurion_helm:'#C89428'};
    return m[item.id] || '#909090';
  },
  _armorColor(item) {
    if (!item) return '#6B4F1A';
    const m = {loin_cloth:'#8B6914', leather_vest:'#7B3A10', chain_mail:'#707070', lorica_segm:'#808090', plate_armor:'#909090'};
    return m[item.id] || '#707070';
  },
  _shieldColor(item) {
    if (!item) return null;
    const m = {wooden_shield:'#8B6914', iron_shield:'#606070', scutum:'#800000', tower_shield:'#404040'};
    return m[item.id] || '#606060';
  },
  _bootsColor(item) {
    if (!item) return '#3A2810';
    const m = {sandals:'#C8A060', iron_greaves:'#808080', roman_greaves:'#909090', hermes_boots:'#C0A010'};
    return m[item.id] || '#808080';
  },
  _darken(hex) {
    // Darken a hex color by ~25%
    const r = parseInt(hex.slice(1,3),16), g = parseInt(hex.slice(3,5),16), b = parseInt(hex.slice(5,7),16);
    return `rgb(${Math.floor(r*0.7)},${Math.floor(g*0.7)},${Math.floor(b*0.7)})`;
  },
};

// ── UI OBJECT (central — all UI functions here, no scope issues) ──
// CombatEngine calls UI.xxx() — this object is defined before CombatEngine is instantiated
const UI = {
  _fromScreen: 'select', // remember where inventory was opened from
  ...
  drawAvatar(canvasId, player) {
    const px = canvasId === 'cv-combat' ? 10 : 12;
    AvatarRenderer.draw(canvasId, player, px);
  },
    ...
    if (isPrize) {
      ...
      wrap.innerHTML = `
        ...
        <div class="prize-card">
          ...
          <div style="font-size:15px;color:var(--tx1);line-height:1.8;">
            HP: ${e.maxHp} &nbsp; ATK: ${e.atk} &nbsp; DEF: ${e.def} &nbsp; SPD: ${e.spd}
          </div>
          <div style="margin-top:10px;font-size:14px;color:var(--gd);">
  ...
  setupCombat(player, enemy) {
    document.getElementById('cb-pname').textContent  = player.name;
    document.getElementById('cb-plevel').textContent = `LVL ${player.level}`;
    document.getElementById('cb-ename').textContent  = enemy.name;
  ...
  refreshCombat() {
    ...
    // Turn counter
    document.getElementById('cb-turn').textContent = `Turn ${ce.turn + 1}`;
    ...
    document.getElementById('cb-phpn').textContent = `${p.hp} / ${p.maxHp}`;

    // Enemy HP
    const ePct = Math.max(0, (e.hp / e.maxHp) * 100);
    document.getElementById('cb-ehp').style.width = ePct + '%';
    document.getElementById('cb-ehpn').textContent = `${e.hp} / ${e.maxHp}`;

    // Distance ruler
    document.querySelectorAll('.dist-pip').forEach(pip => {
      pip.classList.toggle('on', parseInt(pip.dataset.d) === ce.distance);
    });
    document.getElementById('dist-big').textContent = `${ce.distance} ft`;
    ...
    if (myTurn) {
      document.getElementById('ab-attack').disabled  = ce.distance !== 0;
      document.getElementById('ab-ranged').disabled  = ce.distance === 0 || !p.hasRangedWeapon;
      document.getElementById('ab-advance').disabled = ce.distance === 0;
      document.getElementById('ab-backup').disabled  = ce.distance >= 60;
      const cons = p.getConsumables();
      document.getElementById('ab-item').disabled    = cons.length === 0;
      document.getElementById('abn-item').textContent = `${cons.reduce((s,c)=>s+c.qty,0)} items`;
      document.getElementById('ab-mercy').disabled   = (p.hp / p.maxHp) > 0.20;
    }
  },
  ...
  // ── floatDamage — called by CombatEngine ───────────────────
  floatDamage(value, onPlayer, isMiss = false, isCrit = false, isHeal = false) {
    ...
    div.className = 'dmg-float ' + (
      isHeal ? 'heal' : isMiss ? 'miss' : isCrit ? 'crit' : (onPlayer ? 'pd' : 'ed')
    );
  ...
  openItemOverlay() {
    ...
    document.getElementById('ov-item').classList.remove('hidden');
  },

  closeItemOverlay() {
    document.getElementById('ov-item').classList.add('hidden');
    if (G.combat) G.combat._busy = false;
  },

  // ── showMercyOverlay — called by CombatEngine ──────────────
  ...
  renderPost(player, enemy, result) {
    ...
    // Heal section
    document.getElementById('post-hpinfo').textContent = `HP: ${player.hp} / ${player.maxHp}`;
    const smCost   = 30;
    const fullCost = Math.max(20, Math.floor((player.maxHp - player.hp) * 0.8));

    document.getElementById('btn-heal-sm').textContent = `HEAL +40 HP (${smCost}g)`;
    document.getElementById('btn-heal-full').textContent = `FULL RESTORE (${fullCost}g)`;
    document.getElementById('btn-heal-sm').onclick = () => {
    ...
    document.getElementById('btn-heal-full').onclick = () => {
    ...
    // Guard: only wire VIEW FATE when this is a genuine defeat.
    // storeOnly=true (shop-only visits) and mercy-survived must never reach showDeath.
  ...
  renderShop(player) {
    document.getElementById('post-gold').textContent = `Gold: ${player.gold}g`;
  ...
  renderSell(player) {
    ...
    if (!player.inventory.length) {
      list.innerHTML = '<div style="color:var(--tx2);font-size:13px;padding:6px;">Nothing to sell.</div>';
      return;
    }
  ...
  renderInventory(player) {
    ...
    `;
    document.getElementById('inv-gold').textContent = `Gold: ${player.gold}g`;

    const bagEl = document.getElementById('inv-bag');
    bagEl.innerHTML = '';
    if (!player.inventory.length) {
      bagEl.innerHTML = '<div style="color:var(--tx2);font-size:13px;padding:8px;">Bag is empty.</div>';
    } else {
      player.inventory.forEach(({item, qty}) => {
        const row = document.createElement('div');
  ...
  renderHall() {
    ...
    tbody.innerHTML = '';
    if (!legends.length) { emptyEl.style.display = 'block'; return; }
    ...
    legends.forEach((c, i) => {
      const fav = (() => {
        ...
        const e = ENEMY_POOL.find(x => x.id === id);
        return e ? e.name : id;
      })();
  ...
  renderDeath(player, killedBy) {
    document.getElementById('death-sub').textContent =
      `${player.name} was slain after ${player.wins} victories.`;
  ...
  // ── Passed-through action from HTML ───────────────────────
  onAction(action) {
...
const G = {
  ...
  _wireButtons() {
    // Menu
    document.getElementById('btn-play').onclick   = () => this.onPlay();
    document.getElementById('btn-hall').onclick   = () => { UI.renderHall(); UI.show('hall'); };
    document.getElementById('btn-howto').onclick  = () => UI.show('howto');
    document.getElementById('btn-howto-back').onclick = () => UI.show('menu');
    document.getElementById('btn-hall-back').onclick  = () => UI.show('menu');
    document.getElementById('btn-death-back').onclick = () => { UI.renderMenu(); UI.show('menu'); };

    // Load prompt
    document.getElementById('btn-lp-back').onclick = () => UI.show('menu');
    document.getElementById('btn-lp-continue').onclick = () => {
      const d = Save.load();
      if (!d?.currentCharacter) return;
      this.player = Player.fromSave(d.currentCharacter);
      this._goSelect();
    };
    document.getElementById('btn-lp-new').onclick = () => {
      UI.show('creation');
      UI.initCreation();
      this._bindCreate(true);
    };

    // Creation
    document.getElementById('btn-create').onclick = () => this.onCreateCharacter(false);
    ...
    document.getElementById('btn-sel-store').onclick = () => {
      // Show post-battle screen as store (no result banner)
      this.shop.restock(this.player.level);
      UI.renderPost(this.player, {xpReward:0,goldReward:0,isPrizeFight:false}, {storeOnly:true});
      // Replace continue button
      document.getElementById('post-result').textContent = '🏪 ARMOURY';
      document.getElementById('post-result').className   = 'result-banner res-fled';
      document.getElementById('post-rewards').textContent = 'Browse before your next fight.';
      document.getElementById('btn-post-continue').textContent = '← BACK';
      document.getElementById('btn-post-continue').onclick     = () => {
        Save.saveActive(this.player);
        this._goSelect();
      };
      UI.show('post');
    };
    document.getElementById('btn-sel-hall2').onclick = () => { UI.renderHall(); UI.show('hall'); };

    // Inventory back
    document.getElementById('btn-inv-back').onclick = () => {
      Save.saveActive(this.player);
      const from = UI._fromScreen || 'select';
      if (from === 'select') { this._goSelect(); }
      else { UI.show(from); }
    };
  },

  _bindCreate(retirePrev) {
    document.getElementById('btn-create').onclick = () => this.onCreateCharacter(retirePrev);
  },
  ...
  onCreateCharacter(retirePrev) {
    ...
    if (Save.nameExists(name))    { errEl.textContent = 'That name is already taken across all characters!'; return; }

    const newPlayer = new Player(name, gender, skin, style);

    if (retirePrev) {
      const d = Save.load();
      const oldSave = d?.currentCharacter ? Player.fromSave(d.currentCharacter) : null;
      Save.retireAndStartNew(oldSave, newPlayer);
    } else {
  ...
  selectEnemy(idx) {
    ...
    // Clear log before start() writes opening messages
    const logEl = document.getElementById('log-body');
    if (logEl) logEl.innerHTML = '';

    // start() before setupCombat so G.combat.enemy is populated
    // when refreshCombat() is called inside setupCombat
